...
page = st.sidebar.radio(
    "Select a page:", ["About", "Recommendation", "Excel Input"])
...
elif page == "Excel Input":
    # CSV File Upload page
    st.subheader("CSV File Upload for Crop Prediction")
    st.markdown(
        "Upload a CSV file containing soil and environmental data, and the app will predict the best crop.")

    # File upload widget
    uploaded_file = st.file_uploader(
        "Upload a CSV file with crop data", type=["csv"])

    if uploaded_file:
        # Read the uploaded CSV file
        df = pd.read_csv(uploaded_file)

        # Check if the required columns are in the CSV file (adjusted to match column names in your file)
        ...
        if all(col in df.columns for col in required_columns):
            ...
            st.write("### Predictions Added to the Data")
            st.dataframe(df)

            # Allow the user to download the updated CSV file
            output = df.to_csv(index=False)
            st.download_button(label="Download Updated CSV File",
                               data=output, file_name="predicted_crops.csv")
        else:
            st.error(
                "The uploaded file must contain the following columns: N, P, K, temperature, humidity, ph, rainfall.")

    else:
        st.write("Upload a CSV file to get predictions.")

    # Add "Made by" text below content in the Excel Input section
    st.markdown("---")
    st.markdown(text["made_by"])
